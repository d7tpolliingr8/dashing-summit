-- // Aetheris Utility Framework | Solara Compatible (Target Select + Custom Offset Millisecond TP + 3rd Person)
...
if not LibraryLoaded then
    warn("[Aetheris Framework] Failed to load UI library")
...
-- Accent Theme (Cyber Violet)
local CYBER_VIOLET      = Color3.fromRGB(160, 70, 255)
local CYBER_VIOLET_DARK = Color3.fromRGB(90, 20, 180)
local CYBER_BLUE        = Color3.fromRGB(60, 180, 255)
local CYBER_PINK        = Color3.fromRGB(255, 80, 200)
local WHITE             = Color3.fromRGB(255, 255, 255)
local BLACK             = Color3.fromRGB(0, 0, 0)
local GREEN             = Color3.fromRGB(75, 195, 95)
local RED               = Color3.fromRGB(255, 50, 50)

local function ApplyCyberTheme()
    if not Library then return end
    pcall(function()
        Library.AccentColor     = CYBER_VIOLET
        Library.AccentColorDark = CYBER_VIOLET_DARK
        Library.FontColor       = Color3.fromRGB(240, 240, 255)
        Library.MainColor       = Color3.fromRGB(18, 18, 28)
        Library.BackgroundColor = Color3.fromRGB(12, 12, 20)
        Library.OutlineColor    = Color3.fromRGB(40, 40, 55)
...
local function UpdateThirdPerson()
    if not Cfg.ThirdPerson.Enabled then
    ...
    -- Get mouse delta for orbit controls
    local mouseDelta = UserInputService:GetMouseDelta()
    
    -- Smoothly interpolate distance and height
    targetDistance = Cfg.ThirdPerson.Distance
    targetHeight = Cfg.ThirdPerson.Height
    
    if not Cfg.ThirdPerson.Locked then
        -- Allow mouse orbit when not locked
        local rotation = CFrame.Angles(0, -mouseDelta.X * 0.005, 0)
        currentOrbitAngle = currentOrbitAngle or 0
        currentOrbitAngle = currentOrbitAngle - mouseDelta.X * 0.005
        
        local verticalRotation = math.clamp((currentVerticalRotation or 0) - mouseDelta.Y * 0.003, -math.rad(85), math.rad(85))
        currentVerticalRotation = verticalRotation
    end
    
    currentDistance = currentDistance + (targetDistance - currentDistance) * Cfg.ThirdPerson.Smoothness
    currentHeight = currentHeight + (targetHeight - currentHeight) * Cfg.ThirdPerson.Smoothness
    
    -- Calculate camera position
    local camCFrame = CFrame.new(hrp.Position)
    local lookVector = camCFrame.LookVector
    
    -- Apply orbit rotation
    local orbitAngle = currentOrbitAngle or 0
    local verticalAngle = currentVerticalRotation or 0
    
    -- Create offset vector
    local offset = Vector3.new(
        math.sin(orbitAngle) * currentDistance,
        currentHeight,
        -math.cos(orbitAngle) * currentDistance
    )
    
    -- Apply vertical rotation
    local verticalCF = CFrame.Angles(verticalAngle, 0, 0)
    offset = verticalCF * offset
...
-- Initialize orbit variables
local currentOrbitAngle = 0
local currentVerticalRotation = 0
...
RunService:BindToRenderStep("AetherisAim", Enum.RenderPriority.Last.Value, function()
    Camera = Workspace.CurrentCamera
    pcall(DoAim)
end)
...
pcall(function()
    if Window and Window.Holder then
        ...
        strokeGlass.Color = CYBER_VIOLET
        ...
        logoLabel.Name = "AetherisLogoImage"
        ...
        subLabel.Text = "CYBER UTILITY"
        subLabel.TextColor3 = CYBER_VIOLET
        subLabel.TextSize = 9
...
-- 3RD PERSON TAB
-- =============================================
local ThirdPersonGroup = Tabs.Vis:AddRightGroupbox("Third Person Camera")

ThirdPersonGroup:AddToggle("ThirdPersonToggle", { Text = "Enable Third Person", Default = Cfg.ThirdPerson.Enabled })
Toggles.ThirdPersonToggle:OnChanged(function(v)
    Cfg.ThirdPerson.Enabled = v
    if not v then
        Workspace.CurrentCamera.CameraSubject = LP.Character
        Workspace.CurrentCamera.CameraType = Enum.CameraType.Custom
        thirdPersonActive = false
...
ThirdPersonGroup:AddToggle("ThirdPersonLock", { Text = "Lock Camera Rotation", Default = Cfg.ThirdPerson.Locked })
Toggles.ThirdPersonLock:OnChanged(function(v) 
    Cfg.ThirdPerson.Locked = v 
end)

ThirdPersonGroup:AddSlider("ThirdPersonDist", { Text = "Camera Distance", Default = Cfg.ThirdPerson.Distance, Min = 2, Max = 50, Rounding = 1 })
Options.ThirdPersonDist:OnChanged(function(v) 
    Cfg.ThirdPerson.Distance = v 
end)

ThirdPersonGroup:AddSlider("ThirdPersonHeight", { Text = "Camera Height", Default = Cfg.ThirdPerson.Height, Min = -5, Max = 15, Rounding = 1 })
Options.ThirdPersonHeight:OnChanged(function(v) 
    Cfg.ThirdPerson.Height = v 
end)

ThirdPersonGroup:AddSlider("ThirdPersonSmooth", { Text = "Smoothness", Default = Cfg.ThirdPerson.Smoothness, Min = 0.01, Max = 0.5, Rounding = 2 })
Options.ThirdPersonSmooth:OnChanged(function(v) 
    Cfg.ThirdPerson.Smoothness = v 
end)

ThirdPersonGroup:AddButton("Reset Camera", function()
    currentOrbitAngle = 0
    currentVerticalRotation = 0
    currentDistance = Cfg.ThirdPerson.Distance
    currentHeight = Cfg.ThirdPerson.Height
    StarterGui:SetCore("SendNotification", {
        Title = "Aetheris",
        Text = "Camera reset.",
        Duration = 1,
...
-- SETTINGS TAB
-- =============================================
Library:SetWatermark("Aetheris Framework | Solara Compatible")
